<?php
class Weibo_UmaController extends Zend_Controller_Action
{
     ...
     * @return 
     * 前端调用实例
     * window.location.href = "http://iwebsite/weibo/uma/authorize?callbackUrl=http://iwebsite/default/weibo/uma";
     * */
    public function authorizeAction()
    {
    	//http://iwebsite/weibo/uma/authorize?callbackUrl=http://iwebsite/default/uma/uma
    	...
    	$myCallbackUrl = "{$scheme}://{$host}{$path}{$moduleName}/uma/callback?callbackUrl={$callbackUrl}";
    ...
    {
    	//http://iwebsite/weibo/uma/callback?callbackUrl=http://iwebsite/default/uma/uma
    ...
    {
    	//http://iwebsite/weibo/uma/share?jsonpcallback=?&umaId=1233434&uid=1234&pic_url=&content=32323&follow=微博UID&friendNum=3&friends[]=guo
    	try {    		
    		$umaId = trim($this->getRequest()->getParam('umaId',''));//UMAID
     ...
     * 前端调用实例
	     ...
	     $.ajax(
	     {
	     	url:web_path+'weibo/uma/get-user-info',
	     	type:'POST',
	     	data:params,
	     	dataType: "json",
	     	success:function(data) {
    ...
    {
    	//http://iwebsite/weibo/uma/get-user-info?jsonpcallback=?&umaId=1233434&uid=1234
    	try {
    		$umaId = trim($this->getRequest()->getParam('umaId',''));//UMAID
    		$uid = trim($this->getRequest()->getParam('uid',''));//微博UID
     ...
     * 前端调用实例
		     ...
		     $.ajax(
		     {
		     	url:web_path+'weibo/uma/get-friend-list',
		     	type:'POST',
		     	data:params,
		     	dataType: "json",
		     	success:function(data) {
    ...
    {
    	//http://iwebsite/weibo/uma/get-friend-list?jsonpcallback=?&umaId=1233434&uid=1234
    	try {
    		$umaId = trim($this->getRequest()->getParam('umaId',''));//UMAID
    		$uid = trim($this->getRequest()->getParam('uid',''));//微博UID
