...
{
	protected $name = 'iWeixinshop_Order';
	...
	*/
	 ...
	 * 生成订单
	 */
	public function createOrder($OpenId,$ProductId,$body,$gprize,$gnum,$notify_url,
			$attach="",$goods_tag="",$transport_fee=0, $composite_sku_no="", $consignee_province= "", 
			$consignee_city = "", $consignee_area = "", $consignee_name = "", $consignee_address = "", $consignee_tel = "", $consignee_zipcode = "",
			$fee_type=1,$input_charset="GBK",$bank_type="WX", $signType = 'sha1')
	{		
		$data =array();
		//商户系统内部的订单号
		...
		$data ['package'] = $weixinPay->getPackage4JsPay(
				$data ['body'],
				$data ['attach'],
				$data ['out_trade_no'],
				$data ['total_fee'],
	...
	{
		if(is_numeric($trade_state) && intval($trade_state) === 0 && 
				is_numeric($trade_mode) && intval($trade_mode) ===1){
			return true;
		}else{
			return false;
		}
	...
	public function getList4PayFinished($OpenId,$page=1,$limit=10)
	{
    	...
    	//$cacheKey = md5("laiyifenWeixinshop_pay_finished_"."OpenId_".$OpenId."page_".$page."limit_".$limit);
    	...
    	if (empty($list)) {
    		...
    		//$cache->save($list, $cacheKey,array(),300);//利用zend_cache对象缓存查询出来的结果
    	}
    	return $list;
	}
	...
	public function updateOpenId($out_trade_no,$openId)
	{
		...
		$data = array();
		$data['OpenId']= $openId;
		$this->update($query, array('$set'=>$data));
	}
	...
	public function queryTenpayInfo($out_trade_no="")
	{
		...
		//$reqHandler->setParameter("transaction_id", "1218088301201403248351488280");
	...
	public function getList($OpenId,$page=1,$limit=10)
	{
		...
		//$cacheKey = md5("laiyifenWeixinshop_order_"."OpenId_".$OpenId."page_".$page."limit_".$limit);
		//$cache = Zend_Registry::get('cache');
		$list = false;//$cache->load($cacheKey);
		if (empty($list)) {
			$list= $this->find($query,$sort,($page-1)*$limit,$limit);
